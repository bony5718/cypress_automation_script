...
it.only('Testcase 1 - Register User',() => {

cy.visit('https://www.automationexercise.com/')
cy.get('.shop-menu > .nav > :nth-child(4) > a').click()
cy.get('[data-qa="signup-name"]').type("Farzana")
cy.get('[data-qa="signup-email"]').type("tasnim_test4@yopmail.com")
...
cy.get('[data-qa="months"]').select('June')  //dropdown
cy.get('[data-qa="years"]').select('1990')   //dropdown

cy.get('#newsletter').check() //checkbox
cy.get('#optin').check() //checkbox

cy.get('[data-qa="first_name"]').type("Farzana")
cy.get('[data-qa="last_name"]').type("Bony")
cy.get('[data-qa="company"]').type("ACI")
cy.get('[data-qa="address"]').type("1500 MADISON RD")
cy.get('[data-qa="address2"]').type("Apt E")
cy.get('[data-qa="country"]').select('United States')
cy.get('[data-qa="state"]').type("Ohio")
cy.get('[data-qa="city"]').type("Cincinnati")
cy.get('[data-qa="zipcode"]').type("45300")
cy.get('[data-qa="mobile_number"]').type("3287226523")
...
it('Testcase 2 - Login User',() => {
    ...
    cy.visit('https://www.automationexercise.com/')
    cy.get('.shop-menu > .nav > :nth-child(4) > a').click()
    cy.get('[data-qa="login-email"]').type("tasnim_test2@yopmail.com")
...
it('Testcase 4 - Contact Us Form',() => {
    ...
    cy.get('[data-qa="name"]').type("Farzana")
    cy.get('[data-qa="email"]').type("tasnimbony.wedding+test12@gmail.com")
...
it('Testcase 6 - Place Order: Login before Checkout',() => {
    ...
    //cy.login('tasnimbony.wedding+test16@gmail.com', 'Abcd123@');
    
    //Login
    cy.get('[data-qa="login-email"]').type("tasnimbony.wedding+test16@gmail.com")
    cy.get('[data-qa="login-password"]').type("Abcd123@")
    cy.get('[data-qa="login-button"]').click()
